...
export const methods = (req, res, next) => {
  ...
  res.applicationStatus = (user, action) => {
    ...
    switch (action) {
      ...
      case "reject":
        ...
        if (
          user.eventPlannerApplicationStatus === APPLICATION_STATUS.REJECTED
        ) {
          throw new AppError(
            "User has already been rejected. Apply again",
            403,
          );
        }
        break;
  ...
  res.disciplineUserCheck = (user, action) => {
    ...
    switch (action) {
      case "suspendUser": {
        if (isAdmin(user)) {
          throw new AppError(
            "This route is for suspending only non admin users",
            403,
          );
        }
        break;
  ...
  res.deleteManyUserCheck = async (query, User) => {
    ...
    if (query.role === ROLES.SUPER_ADMIN) {
      throw new AppError(
        "Cannot delete a superAdmin user through this route.",
        400,
      );
    }
